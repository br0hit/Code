...
void loop()
{
	...
	rain=analogRead(A3);  // Reading the output of the Rain Sensor
	inductive=digitalRead(inductive_pin);  // Reading the output of the Inductive Proximity sensor

	// Case 1: If it is wet waste: 


	if(rain<500)  // If there is water then the resistance of the metal will decrese and so will the reading 
	{
	   ...
	   // Closing of the top flap
		...
		// Once it is exaclty below it the bottom flap will open and the waste will fall down 
   			...
   			for (pos = 0; pos <= 180; pos += 5 {    // Servo.write defines the angle position of the servo
	...
	else if(inductive==HIGH)  // If there is metal  then the emf of the system  will decrese and so will the reading 
	{
		// First the top flap will open up for the waste to fall in the second bin 

	   		for (pos = 0; pos <= 180; pos += 1) {    // Servo.write defines the angle position of the servo
	    flaptop.write(pos);              
	    delay(20);   // whenever u see time variables in arduino ? delay(time)  the time is in ms                   
	   } 
  ...
  //   		myStepper.step(stepsPerRevolution/6);  //the number given in bracket is number of steps it will take
	...
	else  // In the case when the waste is neither wet/ Nor a metal then it will fall in the Dry bin container
	{
				// First the top flap will open up for the waste to fall in the second bin 
